...
public class FoursquareApiManagerTest
  ...
  @Inject FoursquareApiManager foursquareApiManager;
    ...
    new Runnable() {
      @Override public void run() {
        ...
        BusProvider.getInstance().register(new Object() {
          @Subscribe public void onFoursquareExplore(FoursquareExploreEvent event) {
            signal.countDown();
  ...
  public void testFoursquareSearch() throws Throwable {
    ...
    new Runnable() {
      @Override public void run() {
        foursquareApiManager.search(intent, radius, ll, clientId, clientSecret, "ath", limit,
            sortByDistance, v);
        BusProvider.getInstance().register(new Object() {
          @Subscribe public void onFoursquareExplore(FoursquareSearchEvent event) {
            signal.countDown();
            assertThat(event).isNotNull();
